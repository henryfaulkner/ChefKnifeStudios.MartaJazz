window.ChefMapLibreAnimator = {
    ...
    _log: function (level, msg, data) {
        if (data !== undefined) {
            console[level]('[ChefMapLibreAnimator] ' + msg, data);
        } else {
            console[level]('[ChefMapLibreAnimator] ' + msg);
        }
    },
    ...
    processNearestPointBatch: function (containerDivId, records) {
        ...
        // R4 touch point 1: ChefMapLibre.maps instead of ChefMap.maps
        var map = ChefMapLibre.maps[containerDivId];
        if (!map) {
            this._log('warn', 'processNearestPointBatch: map not found for containerDivId=' + containerDivId);
            return;
        }

        // R4 touch point 2: map.getSource('vehicles') instead of map.sources.getById('vehicles')
        ...
        for (var i = 0; i < records.length; i++) {
            ...
            // R4 touch points 3 & 4: no atlas.data.Feature creation, no ds.add()
            // Vehicle will appear in the next RAF tick's setData call automatically
            if (!existingState) {
                newVehicles++;
            } else {
                updatedVehicles++;
